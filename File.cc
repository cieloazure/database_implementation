...
int Page ::GetNumRecords() { return numRecs; }
...
void Page ::Sort(OrderMaker &sortOrder, bool ascending) {
  ...
  auto c = [&sortOrder, &comp, &ascending](void *i1, void *i2) -> bool {
    Record *i = (Record *)i1;
    Record *j = (Record *)i2;
...
void File ::AddPage(Page *addMe, off_t whichPage) {
  ...
  delete[] bits;
#ifdef F_DEBUG
  cerr << " File: curLength " << curLength << " whichPage " << whichPage
       << endl;
#endif
}
