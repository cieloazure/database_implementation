...
// The fixture for testing class HeapDBFile.
...
/* Unit tests */
TEST_F(HeapDBFileTest, CREATE_SUCCESS) {
...
TEST_F(HeapDBFileTest, CREATE_FAILURE_WHEN_FILE_NAME_IS_TOO_LARGE) {
  string a = "a";
...
TEST_F(HeapDBFileTest, CREATE_FAILURE_WHEN_FILENAME_IS_AN_EMPTY_STRING) {
  DBFile *heapFile = new DBFile();
  fType t = heap;
...
TEST_F(HeapDBFileTest, CREATE_FAILURE_WHEN_FILE_IS_INVALID) {
  DBFile *heapFile = new DBFile();
  EXPECT_FALSE(heapFile->Create("gtest.bin", (fType)4, NULL));
  delete heapFile;
}

TEST_F(HeapDBFileTest, OPEN_SUCCESS) {
  DBFile *heapFile = new DBFile();
  fType t = heap;
...
TEST_F(HeapDBFileTest, OPEN_FAILURE_MISSING_METADATA_FILE) {
  DBFile *heapFile = new DBFile();
  fType t = heap;
  if (heapFile->Create("gtest.bin", t, NULL)) {
    heapFile->Close();
...
TEST_F(HeapDBFileTest, OPEN_FAILURE_ON_INVALID_FILE_TYPE_IN_METADATA_FILE) {
  DBFile *heapFile = new DBFile();
  fType t = heap;
  if (heapFile->Create("gtest.bin", t, NULL)) {
    heapFile->Close();
...
TEST_F(HeapDBFileTest, OPEN_FAILURE_WHEN_A_FILE_DOES_NOT_EXISTS) {
  DBFile *heapFile = new DBFile();
  EXPECT_FALSE(heapFile->Open("gtest2.bin"));
  delete heapFile;
}

TEST_F(HeapDBFileTest, OPEN_FAILURE_WHEN_A_FILE_NAME_IS_INVALID) {
  DBFile *heapFile = new DBFile();
  fType t = heap;
  if (heapFile->Create("gtest.bin", t, NULL)) {
    heapFile->Close();
...
TEST_F(HeapDBFileTest, MOVE_FIRST_WHEN_GET_NEXT_HAVE_BEEN_CALLED_BEFORE) {
...
TEST_F(HeapDBFileTest, MOVE_FIRST_ON_FIRST_RECORD) {
  HeapDBFile *heapFile = new HeapDBFile();
  fType t = heap;
  if (heapFile->Create("gtest.bin", t, NULL)) {
    const char *loadpath = "data_files/lineitem.tbl";
...
TEST_F(HeapDBFileTest, MOVE_FIRST_WITH_NO_RECORDS) {
  HeapDBFile *heapFile = new HeapDBFile();
  fType t = heap;
  if (heapFile->Create("gtest.bin", t, NULL)) {
    // Just opened a file and moving first
...
TEST_F(HeapDBFileTest, MOVE_FIRST_WITHOUT_CREATION) {
  HeapDBFile *heapFile = new HeapDBFile();
  EXPECT_THROW(heapFile->MoveFirst(), runtime_error);
}

TEST_F(HeapDBFileTest, SUCCESSFULLY_ADD_RECORD_INTO_BUFFER) {
  HeapDBFile *heapFile = new HeapDBFile();
...
TEST_F(HeapDBFileTest, LOAD_WITH_NO_OPEN_DBFILE) {
  HeapDBFile *heapFile = new HeapDBFile();
  Schema mySchema("catalog", "lineitem");
...
TEST_F(HeapDBFileTest, LOAD_WITH_NO_EXISTING_TABLEFILE) {
  HeapDBFile *heapFile = new HeapDBFile();
...
TEST_F(HeapDBFileTest, LOAD_SUCCESS) {
  HeapDBFile *heapFile = new HeapDBFile();
  if (heapFile->Create("gtest.bin", heap, NULL) == 1) {
    Schema mySchema("catalog", "lineitem");
    const char *tpch_dir = "data_files/lineitem.tbl";
...
TEST_F(HeapDBFileTest,
       ADD_WHEN_IT_JUST_ADDS_TO_BUFFER_WITHOUT_CREATING_A_NEWPAGE) {
  HeapDBFile *heapFile = new HeapDBFile();
...
TEST_F(HeapDBFileTest, ADD_WHEN_FIRST_FLUSH_TAKES_PLACE) {
  HeapDBFile *heapFile = new HeapDBFile();
  if (heapFile->Create("gtest.bin", heap, NULL)) {
    Schema mySchema("catalog", "lineitem");
    const char *tpch_dir = "data_files/lineitem.tbl";
...
TEST_F(HeapDBFileTest, ADD_WHEN_SUBSEQUENT_FLUSHES_TAKES_PLACE) {
  HeapDBFile *heapFile = new HeapDBFile();
  if (heapFile->Create("gtest.bin", heap, NULL)) {
    Schema mySchema("catalog", "lineitem");
    const char *tpch_dir = "data_files/lineitem.tbl";
...
TEST_F(HeapDBFileTest, ADD_WHEN_AN_EXISTING_PARTIALLY_FILLEDPAGE_IS_UTILIZED) {
  HeapDBFile *heapFile = new HeapDBFile();
  if (heapFile->Create("gtest.bin", heap, NULL)) {
    Schema mySchema("catalog", "lineitem");
    const char *tpch_dir = "data_files/lineitem.tbl";
...
TEST_F(HeapDBFileTest, CLOSE_NORMAL) {
  HeapDBFile *heapFile = new HeapDBFile();
  fType t = heap;
...
TEST_F(HeapDBFileTest, CLOSE_WHEN_FLUSH_BUFFER) {
  HeapDBFile *heapFile = new HeapDBFile();
  fType t = heap;
  if (heapFile->Create("gtest.bin", t, NULL)) {
    Schema mySchema("catalog", "lineitem");
...
TEST_F(HeapDBFileTest, GET_NEXT_WHEN_THERE_ARE_NO_RECORDS_IN_THE_FILE) {
  HeapDBFile *heapFile = new HeapDBFile();
  if (heapFile->Create("gtest.bin", heap, NULL)) {
    Record temp;
...
TEST_F(HeapDBFileTest, GET_NEXT_WHEN_THERE_ARE_RECORDS_IN_THE_FILE) {
  HeapDBFile *heapFile = new HeapDBFile();
  if (heapFile->Create("gtest.bin", heap, NULL)) {
    const char *loadpath = "data_files/lineitem.tbl";
...
TEST_F(HeapDBFileTest, GET_NEXT_WHEN_YOU_REACH_THE_END_OF_THE_FILE) {
  HeapDBFile *heapFile = new HeapDBFile();
  if (heapFile->Create("gtest.bin", heap, NULL)) {
    const char *loadpath = "data_files/lineitem.tbl";
...
TEST_F(HeapDBFileTest, GET_NEXT_WITH_PARAMETERS_GIVEN_CONDITION) {
  HeapDBFile *heapFile = new HeapDBFile();
  if (heapFile->Create("gtest.bin", heap, NULL)) {
    const char *loadpath = "data_files/lineitem.tbl";
...
TEST_F(HeapDBFileTest, GET_NEXT_WITH_PARAMETERS_OR_CONDITION) {
  HeapDBFile *heapFile = new HeapDBFile();
  if (heapFile->Create("gtest.bin", heap, NULL)) {
    const char *loadpath = "data_files/lineitem.tbl";
...
TEST_F(HeapDBFileTest, CLOSE_WITHOUT_OPENING) {
  HeapDBFile *heapFile = new HeapDBFile();
  EXPECT_FALSE(heapFile->Close());
}

// Integration tests
TEST_F(HeapDBFileTest, ADD_AFTER_A_SERIES_OF_GETNEXT) {
  cout << "NOT IMPLEMENTED! TODO FOR VARUN" << endl;
}
TEST_F(HeapDBFileTest, GET_NEXT_AFTER_A_SERIES_OF_ADD) {
  cout << "NOT IMPLEMENTED! TODO FOR VARUN" << endl;
}
TEST_F(HeapDBFileTest, ADD_AFTER_CLOSING_THE_FILE_AND_OPENING_IT_AGAIN) {
  cout << "NOT IMPLEMENTED! TODO FOR VARUN" << endl;
}
TEST_F(HeapDBFileTest, GET_NEXT_AFTER_CLOSING_THE_FILE_AND_OPENING_IT_AGAIN) {
  cout << "NOT IMPLEMENTED! TODO FOR VARUN" << endl;
}
