...
Type Function ::RecursivelyBuild(struct FuncOperator *parseTree,
  ...
  if (parseTree->right == 0 && parseTree->leftOperand == 0 &&
      parseTree->code == '-') {
    ...
    // in this case, we have either a literal value or a variable value, so do a
    // push
    ...
    // now, we have dealt with the case of a unary negative and with an actual
    // value from the record or from the literal... last is to deal with an
    // aritmatic op
  } else {
...
void Function ::GrowFromParseTree(struct FuncOperator *parseTree,
                                  Schema &mySchema) {
