...
QueryPlan *QueryOptimizer::GetOptimizedPlanUtil()
{
  ...
  if (joinPresent)
  {
    ...
    root = GenerateTree(join, *relNameToRelTuple);
  }
  else
  {
    RelationNode *relNode = new RelationNode;
...
BaseNode *QueryOptimizer::OptimizeSelects(BaseNode *root, bool joinPresent)
{
  ...
  if (!joinPresent)
  {
    // Break the select node into multiple nodes for each object in AND list.
    ...
    if (currAnd->rightAnd)
    {
      ...
      while (currAnd)
      {
        ...
        tempAnd->left = currAnd->left;
        tempAnd->rightAnd = NULL;
        cnf->GrowFromParseTree(tempAnd, originalNode->schema, *literal);
