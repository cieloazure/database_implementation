    ...
    for (int whichOr = 0; 1; whichOr++, myOr = myOr->rightOr) {
      ...
      if (myOr->left->left->code == NAME) {
        ...
        if (leftSchema->Find(myOr->left->left->value) != -1) {
          ...
          cout << "ERROR: Could not find attribute " << myOr->left->left->value
               << "\n";
          exit(1);
        }
        ...
        cnf.orList[whichAnd][whichOr].whichAtt1 = numFieldsInLiteral;
        AddLitToFile(numFieldsInLiteral, outRecFile, outSchemaFile,
                     myOr->left->left->value, Double);
        typeLeft = Double;

        // catch-all case
      } else {
        cerr << "You gave me some strange type for an operand that I do not "
                "recognize!!\n";
        exit(1);
      }
      ...
      if (typeLeft != typeRight) {
        cerr << "ERROR! Type mismatch in CNF.  " << myOr->left->left->value
             << " and " << myOr->left->right->value
...
// this is the version that only deals with unary relational selection
// predicates
...
void CNF::GrowFromParseTree2(struct AndList *parseTree, Schema *leftSchema,
  ...
  for (int whichAnd = 0; 1; whichAnd++, parseTree = parseTree->rightAnd) {
    ...
    for (int whichOr = 0; 1; whichOr++, myOr = myOr->rightOr) {
      ...
      // we look it up in the schema
      if (myOr->left->left->code == NAME) {
        std::pair<int, int> statusLeft =
            leftSchema->FindWithStatus(myOr->left->left->value);
        std::pair<int, int> statusRight =
            rightSchema->FindWithStatus(myOr->left->left->value);

        if (statusLeft.first != -1 && statusRight.first != -1) {
          // Found in both
          // Choose one in which join is not present
          if (statusRight.second) {
            cnf.orList[whichAnd][whichOr].operand1 = Left;
            cnf.orList[whichAnd][whichOr].whichAtt1 =
                leftSchema->Find(myOr->left->left->value);
            typeLeft = leftSchema->FindType(myOr->left->left->value);
        ...
        } else if ((statusLeft.first != -1 && statusRight.first == -1) ||
                   (statusLeft.first == -1 && statusRight.first != -1)) {
          // Found in one
          // OK to store
          if (statusLeft.first != -1) {
            cnf.orList[whichAnd][whichOr].operand1 = Left;
            cnf.orList[whichAnd][whichOr].whichAtt1 =
                leftSchema->Find(myOr->left->left->value);
            typeLeft = leftSchema->FindType(myOr->left->left->value);
      ...
      if (myOr->left->right->code == NAME) {
        std::pair<int, int> statusLeft =
            leftSchema->FindWithStatus(myOr->left->right->value);
        std::pair<int, int> statusRight =
            rightSchema->FindWithStatus(myOr->left->right->value);
