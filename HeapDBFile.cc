...
HeapDBFile::~HeapDBFile() { delete persistent_file; }
...
int HeapDBFile::CopyBufferToPage(Page *buffer, Page *flush_to_page,
...
void HeapDBFile::CheckIfFilePresent() {
  if (!is_open) {
    throw runtime_error(
...
bool HeapDBFile::CheckIfCorrectFileType(fType type) {
  switch (type) {
    case heap:
    case sorted:
    case tree:
...
bool HeapDBFile::CheckIfFileNameIsValid(const char *file_name) {
...
int HeapDBFile::GetNumRecsInBuffer() { return buffer->GetNumRecords(); }

bool HeapDBFile::WillBufferBeFull(Record &to_be_added) {
  return buffer->IsPageFull(&to_be_added);
}

off_t HeapDBFile::GetNumPagesInFile() { return persistent_file->GetLength(); }

off_t HeapDBFile::GetCurrentReadPageIndex() { return current_read_page_index; }

off_t HeapDBFile::GetCurrentWritePageIndex() {
